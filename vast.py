...
@parser.command(
    ...
    argument("--exclude", help="exclude files matching PATTERN", type=str),
...
def copy(args: argparse.Namespace):
    ...
    if (r.status_code == 200):
        ...
        if (rj["success"]) and ((src_id is None) or (dst_id is None)):
            ...
            exclude = f" --exclude={args.exclude} " if args.exclude is not None else ""
            if (src_id is None):
                #result = subprocess.run(f"mkdir -p {src_path}", shell=True)
                remote_port = rj["dst_port"]
                remote_addr = rj["dst_addr"]
                cmd = f"sudo rsync -arz -v --progress {exclude} --rsh=ssh -e 'sudo ssh -i {identity} -p {remote_port} -o StrictHostKeyChecking=no' {src_path} vastai_kaalia@{remote_addr}::{dst_id}/{dst_path}"
                print(cmd)
                ...
                cmd = f"sudo rsync -arz -v --progress {exclude} --rsh=ssh -e 'sudo ssh -i {identity} -p {remote_port} -o StrictHostKeyChecking=no' vastai_kaalia@{remote_addr}::{src_id}/{src_path} {dst_path}"
                print(cmd)
                result = subprocess.run(cmd, shell=True)
